 ...
 * SELECTED FINDINGS
 * =============================================================================
 * Detail behind the "500+ vulnerabilities" counter in bugbounty.ts. That file
 * holds the CV's headline figures; this one shows what the work actually looks
 * like, which is what a mentee (or a client) needs to see.
 *
 * RULES, PLEASE PRESERVE THEM:
 *
 *   · Targets are described by SECTOR, never by client name. Everything here is
 *     either under NDA or under coordinated disclosure that does not name the
 *     target. The named organisations in bugbounty.ts are Hall-of-Fame
 *     acknowledgements — do NOT attach a specific vulnerability to one of them
 *     unless there is a public report proving it.
 *
 *   · `disclosure` stays null unless you have a real, verifiable URL. An
 *     invented HackerOne link is the fastest way to lose this audience.
 *
 *   · The CVSS vector must match the score. People here read vectors.
 *
 *   · Impact first: what an attacker could do, not which tool found it.
 */
...
export type Finding = {
  ...
  cvss: number;
  vector: string;
  /** Sector descriptor, not the client name. */
  target: string;
  impact: string;
...
/** Derived from the data, so a new class appears in the filter automatically. */
export const findingClasses = Array.from(
  new Set(findings.map((f) => f.vulnClass)),
).sort() as VulnClass[];

export const findingsCopy = {
  eyebrow: "Selected findings",
  title: "What the 500+ actually looks like",
  description:
    "A sample of disclosed work, described by sector rather than client name. This is the level of detail I teach people to reach — and to write up.",
};
